...
public class HttpConnector {
	...
	private static Header DEFAULT_HEADER = new BasicHeader(HttpHeaders.USER_AGENT, "wain365-pay");
	...
	public synchronized static CloseableHttpClient initHttpclient() {
		...
		// 证书路径，证书密码一个为空，就不加载ssl
		if (StringUtils.isEmpty(certPath) || StringUtils.isEmpty(certPwd)) {

			logger.info("do not need init cert for ssl,certPath is: " + certPath + ",certPwd:" + certPwd);
			return null;
		}
		...
		try {
			...
			InputStream instream = HttpConnector.class.getClass().getResourceAsStream(certPath);
			if (instream == null) {
				logger.error("failed to load cert file: {}", certPath);
				return null;
			}
			...
			try {
				...
				logger.info("finish load keyStore: {}", certPwd);
			} finally {
				instream.close();
			}

			// Trust own CA and all self-signed certs
			sslcontext = SSLContexts.custom().loadKeyMaterial(keyStore, certPwd.toCharArray()).build();
			// Allow TLSv1 protocol only
			sslsf = new SSLConnectionSocketFactory(sslcontext, new String[] { "TLSv1" },
					null, SSLConnectionSocketFactory.BROWSER_COMPATIBLE_HOSTNAME_VERIFIER);

			logger.info("end init cert for ssl.");

	

		} catch (Exception e) {
			logger.error("SSLConnectionSocketFactory load ssl  cert failed!", e);

			throw new RuntimeException("SSLConnectionSocketFactory load ssl  cert failed!");
		}
		...
		httpclient = builder.setConnectionManager(connManager).setRetryHandler(DefaultHttpRequestRetryHandler.INSTANCE)
	 ...
	 * @return
	 */
	private static SSLConnectionSocketFactory initSSLHttpFactory() {
		logger.info("begin init cert for ssl");
	...
	public static String connectPostHttps(String url, Map<String, String> params) {
		...
		try {
			...
			CloseableHttpResponse response = getHttpClient().execute(httpsPost);
			try {
				int statusCode = response.getStatusLine().getStatusCode();
				if (statusCode != HttpStatus.SC_OK) {
					httpsPost.abort();
	 ...
	 * @throws Exception
	 */
	public static String sendHttpPostWithXml(String domainUrl, String requestXml) throws Exception {

		logger.info("sendHttpPostWithXml,[domainUrl]:{},[requestXml]: {}", domainUrl, requestXml);
		...
		try {
			...
			try {
				...
				if (statusCode != HttpStatus.SC_OK) {
					httppost.abort();
					logger.error("[sendHttpPostWithXml] error, url : {}  , params : {},  status :{}", domainUrl,
							requestXml, statusCode);
					return "";
				}
...
//				HttpEntity entity = response.getEntity();
				...
				try {
					if (null != entity) {
						...
						logger.info("sendHttpPostWithXml,[domainUrl]:{},[requestXml]: {},[responseContent]:{}",
								domainUrl, requestXml, responseContent);
	 ...
	 * @throws Exception
	 */
	public static String sendHttpPostWithXml2(String domainUrl, String requestXml) throws Exception {

		SSLConnectionSocketFactory factory = initSSLHttpFactory();
