...
public class QRCodeUtil
{
	...
	public static BufferedImage createImageFaster(String content, String logoPath, int size, String remarks)
		throws Exception
	{
		...
		BufferedImage image = createImageFaster(content, logoImage, size, remarks);
		return image;
	}

	public static BufferedImage createImageFaster(String content, Image logoImage, int size) throws Exception
	{
		return createImageFaster(content, logoImage, size, "");
	}
	...
	public static BufferedImage createImageFaster(String content, Image logoImage, int size, String remarks)
		throws Exception
	{
		Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
		hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.H);
		...
		{
			imageHeight = (int) (height + FONT_SIZE);
		}
		...
		if (remarks != null && remarks.length() > 0)
		{
			for (int y = height; y < imageHeight; y++)
			{
				for (int x = 0; x < width; x++)
				{
					dstbuf[y * width + x] = 0xFFFFFFFF;
				}
			}
		}
		if (logoImage != null)
		...
		//
		// String remarks = "这是一个有点长的客户的名称";
		if (remarks != null && remarks.length() > 0)
		{
			Graphics2D g2d = image.createGraphics();
			g2d.setColor(Color.black);
			// g2d.setFont(g2d.getFont().deriveFont(FONT_SIZE));
			g2d.setFont(new Font("宋体", Font.PLAIN, (int) FONT_SIZE));
			g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
			g2d.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
			g2d.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
			Rectangle2D rc = g2d.getFontMetrics().getStringBounds("国", g2d);
			int unitWidth = (int) Math.ceil(rc.getWidth());
			int unitHeight = (int) Math.ceil(rc.getHeight());
			int textWidth = unitWidth * remarks.length();
			//
			int x = Math.max((width - textWidth) / 2, 0);
			int y = height;
			g2d.drawString(remarks, x, y);
		}
		return image;
	}
