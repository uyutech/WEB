...
public class HotTagsUpdateTask {
	...
	public void run() {
		...
		int batchNum = 30;
		int recentNumLimit = 15;
		
		//48小时最火的标签
		List<Long> hotTagRecently = userFollowTagsMappingDAO.queryHotTagsRecently(recentNumLimit);
		
		//总热度最高的标签 135条
		List<Long> totalHotTags = userFollowTagsMappingDAO.queryHotTagsByPage(0, 135, hotTagRecently);
		...
		for(Long id:allIds) {
			...
			if(tag != null) {
				...
				if(suggestList.size() >=batchNum) {
					...
					redisHelper.expire(hashKey, 30, TimeUnit.HOURS);
